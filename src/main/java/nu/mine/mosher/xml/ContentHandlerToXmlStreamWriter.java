...
public class ContentHandlerToXmlStreamWriter extends DefaultHandler {
    ...
    public void startElement(final TagName tagName, Attributes atts) throws SAXException {
        String namespaceURI = tagName.namespace();
        String localName = tagName.tag();
        String qName = tagName.legacyTag();
        ...
        try {
            ...
            while (!prefixBindings.isEmpty()) {
                uri = prefixBindings.pop();
                prefix = prefixBindings.pop();
                if (prefix.length() == 0) {
     ...
     *
     * @param atts attributes from the SAX event
     */
    private void writeAttributes(Attributes atts) throws XMLStreamException {
        for (int i = 0; i < atts.getLength(); i++) {
            ...
            if (!prefix.equals("xmlns")) { // defend against broken transformers that report xmlns decls as attrs
